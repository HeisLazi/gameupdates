...
/* ==========================================================================
 * app.jsx · App shell · State persistence · Study Mode · Exam · Topnav
 *
 * Splice position: ROOT. Every other src/build/*.jsx is concatenated and
 *                  spliced into THIS file by build_html.py, immediately
 *                  AFTER the React destructure line above. Splicing earlier
 *                  triggers a TDZ error ("Cannot access 'React' before
 *                  initialization") that silently drops the entire script.
 * Defines:  STORAGE_KEY, SESSION_KEY, PDF_DB_NAME, pdfEntryKey,
 *           serializeLibraryPdf(s), savePdfBlob, loadPdfBlob, deletePdfBlob,
 *           dataUrlToBlob, defaultArenaState, normalizeArena, loadState,
 *           saveState, loadSession, saveSession, clearSession, defaultState,
 *           DAY_MS, dayKey, shuffle, updateStreak, scheduleNext, xpForRating,
 *           levelFromXp, STOP, tokens, keywordScore, renderNotes, runCode,
 *           PomodoroWidget, StreakXp, DeadlinesWidget, StreakChip, PomoChip,
 *           ConceptTeach, QuizRunner, Home, StudyPicker, ExamPicker,
 *           DailyView, Stats, DeadlinesView, GradesView, Modules, App,
 *           ReactDOM root mount.
 * Reads:    everything that's spliced in by build_html.py — but only at
 *           render time. Top-level statements above the splice point cannot
 *           reference any spliced helper.
 *
 * Topnav (Study side, post-consolidation 2026-05-09): 4 parents
 *   Learn (Home · Study · Daily · Review · Mixed) ·
 *   Exam (Exam · Pass Sprint) · Library · Progress (Stats · Deadlines ·
 *   Grades · Modules). Sub-tab strip below the topbar surfaces the
 *   children of the active parent.
 *
 * Edit when: changing top-level state shape, persistence, the topnav,
 * Study Mode flow, Exam flow, or routing into Arena (which is just
 * <ArenaView/> from arena_views.jsx).
 * ========================================================================== */
