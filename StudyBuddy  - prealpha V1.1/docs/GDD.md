...
> Living document. Last updated 2026-05-09 (mode-consolidation + folder restructure).
> Supersedes `studybuddy_src/StudyBuddy_GDD.md` (also: `studybuddy_src/` was renamed to `src/` on 2026-05-09).

## CHANGELOG

**2026-05-09 — Mode consolidation + project restructure**

UI / mode trim (sections 3, 6, 7, 11 below now partially stale — read this changelog first):
- **Lock In Mode killed.** Was a 48-line wrapper over CombatScreen; Practice covers the same use case. `lockin.jsx` is now an empty tombstone, removed from the splice list.
- **Cram Mode renamed to "Blitz" in the UI** (state shape stays `state.arena.cram` for backwards compat).
- **Pass Mode** (`study_pass.jsx`) is no longer a top-level Study tab — folded under Exam as a deadline-aware sub-action.
- **Arena hub: 13 buttons → 5.** New layout: BUILD (Loadout · Workshop) · PLAY (Blitz · Raids) · World stages render inline. Old phase strings (`armory`, `tree`, `hotbar`, `pets`, `passives`, `forge`, `librarian`, `exchange`) auto-redirect to the new wrappers.
- **Study topnav: 11 buttons → 4.** New layout: Learn · Exam · Library · Progress, with sub-tab strip below the topbar.
- **New tabbed wrappers** in `arena_views.jsx`: `LoadoutView` (Armory · Tree · Hotbar · Pets · Passives) and `WorkshopView` (Forge · Librarian · Exchange — Bestiary lives inside Librarian).

Project restructure (section 4.1 below now stale — read `CONTEXT.md` for the new file map):
- `studybuddy_src/` → `src/`
- `design_handoff_studybuddy/` → `design/`
- `qa_screens/` → `test/qa_screens/`
- New folders: `docs/` (this file lives here now), `test/`, `design_handoff_v2/`
- Deleted: `PATCH_BUNDLE/`, `PATCH_BUNDLE_LOCATION.md`, `StudyBuddy.jsx` (root prototype), `study_buddy.jsx` (root prototype), `_sync_test.tmp`
- New tester entry: `START_HERE.md`. New build wrappers: `build.bat` / `build.sh`. Migration script: `migrate.bat`.

Data hygiene:
- `LOOT_TABLE` in `arena_runtime.jsx` had a duplicate `scholar_eye` key alongside `scholars_eye` — deduped 2026-05-09. `bonuses.jsx` AUGMENT_EFFECTS key renamed to match. (NB: the two entries advertise different effects — crit chance vs revealEnemyHp — that mismatch is a pending balance call.)

Splice order (post-consolidation):
```
extras → world → bonuses → skills → caveman → enemies → multimob
       → cram → study_pass → arena_runtime → raids → combat → arena_views
```
(Was: ...→ cram → lockin → study_pass →... — `lockin.jsx` removed.)
...
| **Blitz Mode** (was "Cram") — 10-min wave survival, 3-ability draft, wave-10 boss | ✅ Shipped (Day 11 rework included) | `cram.jsx` | Real-time timer. Pick 3 starter abilities → +1 every 3 waves cleared. Boss spawns at wave 10 OR 10:00 elapsed (no early skip). 4× XP, 0.5× HP. Legendary augments only drop here. UI brand "Blitz"; state key `state.arena.cram`. |
...
| Forge (craft / upgrade augments) | ✅ Shipped (Day 6) | `arena_runtime.jsx`, `arena_views.jsx` ForgeView | Sharding (tier-scaled), crafting (shards + insights), upgrading (+~20% per level, max +60%). Now lives under Workshop tab. |
| Level-up choice rewards (passives / stats / abilities) | ✅ Shipped (Day 6) | `arena_runtime.jsx` REWARD_POOL + `combat.jsx` modal | Modal fires on level transition. 3 picks per level from weighted pool (stat / passive / unlock). |
| Passives tab (read-only view of all active passives) | ✅ Shipped (Day 6) | `arena_views.jsx` PassivesView | Lives as a tab inside the new LoadoutView. Sections: tree · sockets · pet · prestige. |
| Forbidden Exchange (risk-reward shop) | ✅ Shipped (Day 7) | `arena_runtime.jsx` EXCHANGE_OFFERS + `arena_views.jsx` ExchangeView | 6 offers (Bone Tax, Memory Burn, All-In Gamble, Pet Sacrifice, Black Box Forge, Hot Streak Trade). 1% data-corruption chance per offer. Now lives under Workshop tab. |
| Pet XP system + 5-level progression | ✅ Shipped (Day 7) | `bonuses.jsx` PET_XP_THRESHOLDS + petLevelFromXp + addPetXp | Pets level 1→5 via fight participation, kills, daily plan. Hooks scale with level (e.g. Buffer heal 10→22). |
| Stats-as-buffs (lifetime stats grant gameplay bonuses) | ✅ Shipped (Day 7) | `bonuses.jsx` STAT_BUFF_TIERS + getStatBuffs | 5 ladders: lifetime correct → +Max HP (cap +20), kills → +flat dmg (cap +10), reading → +Insight cap (cap +5), fights won → +momentum cap (cap +3), dmg dealt → +crit (cap +10%). Layered on top of getCombinedStats. |
| **Mode consolidation** (Lock In killed, Arena hub 13→5, Study topnav 11→4) | ✅ Shipped (2026-05-09, ad-hoc) | `arena_views.jsx`, `app.jsx`, `arena.css`, `lockin.jsx` (tombstoned) | See CHANGELOG above. |
| **Folder restructure for testers** (`studybuddy_src/` → `src/` etc., docs/ + test/ hubs, START_HERE.md, build.bat, migrate.bat) | ✅ Shipped (2026-05-09, ad-hoc) | `migrate.bat`, `README.md`, `START_HERE.md`, `CONTEXT.md`, `docs/`, `test/` | See CHANGELOG above. |
| Prestige Perks | ❌ Day 8+ | — | Post-game meta progression. |
| Tutorial module + 4 stock modules | ✅ Shipped (Day 9) | `src/modules_all.json` | Tutorial teaches the game. Basic Maths / English / Science / Computer Science as stock content. |
