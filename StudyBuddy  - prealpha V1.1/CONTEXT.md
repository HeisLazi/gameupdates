...
├── arena_views.jsx      ← arena UI (hub, stats, passives, codex, sanctuary)
├── arena.css            ← arena-only styles
├── bonuses.jsx          ← getCombinedStats core + prestigeFlags
├── caveman.jsx          ← caveman path (LAST in splice — monkey-patches getCombinedStats)
├── combat.jsx           ← combat screen, enemy render, intent badges (line ~412)
├── cram.jsx             ← Cram Mode (will become Blitz if fork 1 accepted)
...
**Splice order (must match):** `bonuses → enemies → multimob → cram → skills → caveman`. Caveman last.
