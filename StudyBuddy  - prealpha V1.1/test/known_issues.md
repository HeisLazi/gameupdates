...
**Status:** **fixed (2026-05-09 wave 2)**
**Reported:** 2026-05-09 by claude
**Where:** `src/build/arena_runtime.jsx` LOOT_TABLE + `src/build/bonuses.jsx` AUGMENT_EFFECTS
**Fix:** Deleted `scholar_eye` from LOOT_TABLE; renamed `scholar_eye` → `scholars_eye` in AUGMENT_EFFECTS so the socket-time effect is keyed correctly. Rebuild via `build.bat` to refresh the bundled HTML.
**Follow-up open:** the LOOT_TABLE entry advertises `Crit chance +10%` while the AUGMENT_EFFECTS entry sets `revealEnemyHp:true`. Pick one effect and unify in a balance pass.
